...
export function AdminUsers({ currentUserId, initialUsers }: { currentUserId: string; initialUsers: AdminUser[] }) {
  ...
  return <><div className="page-heading"><div><h1>管理员</h1><p>管理后台账号、角色和账号状态。</p></div><Button className="primary" onClick={() => { setError(""); setEditing(null); }}><Plus size={17}/>新增管理员</Button></div>
    ...
    {editing !== undefined && <AdminModal item={editing} error={error} currentUserId={currentUserId} onClose={() => setEditing(undefined)} onSave={save}/>} {editing === undefined && error && <p className="form-error">{error}</p>}</>;
}
...
function AdminModal({ item, error, currentUserId, onClose, onSave }: { item: AdminUser | null; error: string; currentUserId: string; onClose: () => void; onSave: (admin: Omit<AdminUser, "createdAt"> & { password: string }) => void }) {
  const isSelf = Boolean(item && item.id === currentUserId);
  const roleOptions = isSelf ? [{ value: "super_admin" as const, label: "超级管理员" }] : [{ value: "content_admin" as const, label: "内容管理员" }];
  const statusOptions = isSelf ? [{ value: "active" as const, label: "启用" }] : [{ value: "active" as const, label: "启用" }, { value: "disabled" as const, label: "停用" }];
  const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = new FormData(event.currentTarget); void onSave({ id: item?.id || "", name: String(form.get("name")).trim(), email: String(form.get("email")).trim(), password: String(form.get("password")), role: form.get("role") as AdminUser["role"], status: form.get("status") as AdminUser["status"] }); };
  return <Dialog open onOpenChange={open => !open && onClose()}><DialogContent><DialogHeader><DialogTitle>{item ? "编辑管理员" : "新增管理员"}</DialogTitle><DialogCloseButton/></DialogHeader><form className="form-grid" onSubmit={submit}><Label>姓名<Input name="name" defaultValue={item?.name} required/></Label><Label>邮箱<Input name="email" type="email" defaultValue={item?.email} required/></Label><Label className="full">密码<Input name="password" type="password" minLength={8} required={!item} placeholder={item ? "留空则不修改" : "至少 8 位"} autoComplete="new-password"/></Label><Label>角色<FilterSelect name="role" defaultValue={item?.role || "content_admin"} options={roleOptions}/></Label><Label>状态<FilterSelect name="status" defaultValue={item?.status || "active"} options={statusOptions}/></Label>{isSelf && <p className="form-hint full">不能降低本人角色、停用本人或新增超级管理员</p>}{!item && <p className="form-hint full">新增的管理员均为内容管理员</p>}{error && <p className="form-error full">{error}</p>}<div className="modal-actions full"><Button type="button" variant="secondary" className="secondary" onClick={onClose}>取消</Button><Button className="primary">保存管理员</Button></div></form></DialogContent></Dialog>;
}
